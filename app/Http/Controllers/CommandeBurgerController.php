...
class CommandeBurgerController extends Controller {
    ...
    public function update(Request $request, CommandeBurger $cartItem)
    {
        ...
        if ($cartItem->burger->stock < $quantity) {
            return back()->with('error', 'Stock insuffisant');
        }

        $cartItem->update(['quantite' => $quantity]);
    ...
    public function checkout(Request $request)
    {
        ...
        $commande = CommandeBurger::create([
            ...
            'status' => 'en attente',
            'total' => $cartItems->sum(function ($item) {
                return $item->burger->price * $item->quantity;
            })
        ]);

        // Ajouter les burgers à la commande et mettre à jour le stock
        foreach ($cartItems as $item) {
            $commande->burgers()->attach($item->burger_id, ['quantity' => $item->quantity]);
            $item->burger->decrement('stock', $item->quantity);
            $item->delete();
        }
